...
class InforContactController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(count($infor_contact)>0){
            return redirect()->route('admin.infor_contact.index')->with('error', 'Contact Info');
        }
        else {
            if(empty($request->id_inforcontact))
            {
                ...
                $infor_contact->save();
                return redirect()->route('admin.infor_contact.index')->with('success', 'Information created successfully');
            }
            ...
            $infor_contact->map = $request->map;
            $infor_contact->save();
            return redirect()->route('admin.infor_contact.index')->with('success', 'Information has been successfully updated');
        }
    }
