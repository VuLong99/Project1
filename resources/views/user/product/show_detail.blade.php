                    ...
                    <div class="category-tab shop-details-tab"><!--category-tab-->
                        <div class="col-sm-12">
                            <ul class="nav nav-tabs">
                                <li class="active"><a href="#content" data-toggle="tab">Description</a></li>
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane fade" id="reviews" >
                                <div class="response-area">
                                    <ul class="media-list">
                                        ...
                                        @if(!$isAdmin && !$isBought)
                                            <li class="active">
                                                <div>
                                                    @if(Auth::check())
                                                        <span style="color: #FE980F !important;">Purchase</span>
                                                    @else
                                                        <a class="review-login" href="{{url('/login?detail=true')}}">Login</a> and <span style="color: #FE980F !important;">purchase</span> 
                                                    @endif
                                                    to comment on products
                                                </div>
                                            </li>
                                            <hr>
                                        @endif
                                        ...
                                        <li class="media">
                                            <div class="form-group pl-10-custom">
                                                ...
                                                <div class="error error-comment error-comment_text">Enter the text comment</div>
                                                <button class="btn btn-custom" @click="comment()">Comment</button>
                                            </div>
                                        </li>
                                        @endif
                                        <li class="media"  v-if="comments.length > 0" v-for="(comment, index) in comments">
                                            ...
                                            <div class="media-body">
                                                ...
                                                @if(Auth::check() && $isBought || $isAdmin)<span class="cursor title-reply" @click="showReply(comment.id)">Answer</span>@endif
                                                @if(Auth::check() && $isBought)<span  v-if="checkPermission(comment.user_id)" class="cursor title-reply pl-10px" @click="deleteComment(comment.id)">Delete</span>@endif
                                                @if($isAdmin)<span class="cursor title-reply pl-10px" @click="deleteComment(comment.id)">Delete</span>@endif
                                                @if(Auth::check() && $isBought || $isAdmin)
                                                <ul class="form-child d-none" :id="getClassForm(comment.id)">
                                                    <li class="media">
                                                        <div class="form-group pl-10-custom">
                                                            ...
                                                            <div class="error error-comment" :class="getErrorName(comment.id)">Enter the text comment</div>
                                                            <button class="btn btn-custom" @click="replyComment(comment.id)">Conment</button>
                                                        </div>
                                                    </li>
                                                </ul>
                                                @endif  
                                            ...
                                            <ul v-if="comment.children.length > 0">
                                                <li class="media second-media" v-for="(item, index) in comment.children">
                                                    ...
                                                    <div class="media-body">
                                                        ...
                                                        @if(Auth::check() && $isBought)<span  v-if="checkPermission(item.user_id)" class="cursor title-reply pl-10px" @click="deleteComment(item.id,item.parent_id)">Delete</span>@endif
                                                        @if($isAdmin)<span class="cursor title-reply pl-10px" @click="deleteComment(item.id,item.parent_id)">Delete</span>@endif
                                                    </div>
                                                </li>
                                            </ul>
                                        </li>
                                    ...
                                    @if(count($comments) >= 5)
                                    <div class="form-group pl-10-custom">
                                        <center><button class="btn btn-custom" v-if="!all" @click="showMore()">See more</button></center>
                                    </div>
                                    @endif
                                </div><!--/Response-area-->
                            </div>
