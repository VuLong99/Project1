        ...
        <div class="container-fluid">
          ...
          <header> 
            <div class="box-header">
              ...
              @if($isIndex ?? false) <a href="{{ route('admin.order.order_online_unpaid') }}" class="btn  btn-info">Unpaid online orders</a>@endif
            </div>
          </header>
          <div class="row">
            <div class="col-lg-12">
                  ...
                  <div class="card-body card-form" style="padding:0;">
                    <form class="form-inline" style="float: right;" method="GET" action="{{$url}}">
                      ...
                      <div class="form-group">
                        ...
                        <input id="inlineFormInputGroup" value="{{Request::get('search')}}" name="search" type="text" placeholder="Enter your order code" class="mr-3 form-control form-control">
                      </div>
                      <div class="form-group">
                    ...
                    <table class="table table-striped table-hover">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>{{$row->id}}</td>
                          <td>
                            <div>FullName: {{ $row->fullname }}</div>
                            <div>Email: {{ $row->email }}</div>
                            <div>Phone Number: {{ $row->phone }}</div>
                            <div>Address: {{ $row->address }}</div>
                          </td>
                          <td>{{ getMethodPayment($row->payment)}}</td>
                          <td>{{ format_price($row->total) }}&#8363;</td>
                          <td><button id="btn-paid-{{$row->id}}" class="btn btn-paid {{$row->payment->status ? "btn-success" : "btn-danger"}}" data-id="{{$row->id}}" >{{$row->payment->status ? "Paid" : "Unpaid"}}</button></td>
