        ...
        <div class="col-lg-12">
          <div class="card">
            <div class="card-header" style="display:flex;">
              {{-- Form delete --}}
              <form id="delete-post" action="{{ route('admin.post.delete') }}" method="POST" style=" display: inline;">
                @csrf
                @method('DELETE')
                <input type="hidden" name="list_id" id="list_id" value="">
                <button disabled type="submit" class="btn btn-danger btn-delete">Delete post</button>
              </form>

              {{-- Form search desktop --}}
              <div class="card-body card-form"  style="padding:0;">
                <form class="form-inline" style="float: right;" method="GET" action="{{route('admin.post.index')}}">
              ...
              {{-- Form search desktop --}}
              <button class="btn btn-primary btn-create" data-url="{{ route('admin.post.create') }}">Create post</button>
            </div>
                ...
                <table class="table table-striped table-hover" id="myTable2">
                  <thead>
                    <tr>
                      ...
                      <th class="cursor" onclick="sortTable(0)">Name</th>
                      <th class="cursor" onclick="sortTable(1)">Author</th>
                      <th class="cursor" onclick="sortTable(2)">description</th>
                      <th>ContentContent</th>
                      <th></th>
                      <th></th>
                      <th></th>
                    </tr>
...
@section('script')
  <script >
    ...
    $('.edit').click(function(){
      let id = $(this).data('id');
      window.location.href= location.origin + '/admin/post/edit/' + id;
    });

    //Click info
    $('.info').click(function(){
      let id = $(this).data('id');
      window.location.href= location.origin + '/admin/post/' + id + "/detail";
    });
