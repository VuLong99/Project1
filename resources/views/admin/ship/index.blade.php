...
<section class="charts">
  <section>
        <div class="container-fluid">
          <!-- Page Header-->
          <header> 
            <h1 class="h3 display">Transport fee</h1>
          </header>
          <div class="row">
            <div class="col-lg-12">
              <div class="card">
                <div class="card-header" style="display:flex;">
                  ...
                  <form id="delete-ship" action="{{ route('admin.ship.delete') }}" method="POST" style=" display: inline;">
                    ...
                    <button disabled type="submit" class="btn btn-danger btn-delete">Remove fee</button>
                  </form>
                 ...
                 {{-- Form search desktop --}}
                  <button class="btn btn-primary btn-create">Create fee</button>
                </div>
                ...
                <form class="form-inline " style="float: right;" method="GET" action="{{route('admin.ship.index')}}">
                  <div class="form-group">
                    <label for="inlineFormInputGroup" class="sr-only">NameName</label>
                    <input id="inlineFormInputGroup" value="{{Request::get('search')}}" name="search" type="text"  placeholder="Tỉnh/Thành phố" class="mr-3 form-control form-control">
                  </div>
                  <div class="form-group">
      ...
      <div role="document" class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 id="exampleModalLabel" class="modal-title">Create role</h5>
          ...
          <div class="modal-body">
            <form id="form-ship" action="{{route('admin.ship.store')}}" method="POST">
              ...
              <div class="form-group">
                ...
                <input type="text"  value="" placeholder="Enter shipping fee" name="fee" id="fee" class="form-control">
                <div class="error error-fee">Enter shipping fee</div>
              </div>
            </form>
          </div>
...
@section('script')
  <script >
    ...
    $('.btn-create').click(function(){
      ...
      $('.modal-title').html('Create shipping fee');
    ...
    $('#save').click(function(){
      ...
      if($('#city').val() == '')
      {
        $('.error-city').html('Choose the city');
      ...
      if($('#fee').val() == '')
      {
        $('.error-fee').html('Enter the minimum price');
        $('.error-fee').css('display','block');
        flag = true;
      }
      else if($('#fee').val() > 10000000000|| $('#fee').val() < 1000)
      {
        $('.error-fee').html('Enter the price from 1000 to 10 billion VND');
        $('.error-fee').css('display','block');
        flag = true;
      }
      else
