...
<section class="charts">
  <section>
        <div class="container-fluid">
          ...
          <header> 
            <div>
              <h1 class="h3 display ">Bills statistics</h1>
            </div>
          </header>
          <div class="row">
            <div class="col-lg-12">
                    ...
                    <div class="col-sm-12" style="margin: 0;display:flex">
                      <form id="form-sort-month"  action="{{route('admin.dashboard.revenue_detail')}}" method="get">
                        ...
                        <div class="form-group">
                          <button type="submit" class="btn btn-filter btn-report-filter btn-primary">Filter</button>
  ...
  <script >
    ...
    $('#form-sort-month').submit(function(e){
      ...
      if(Date.parse(fromDate) > Date.now()) {
        $('.error-from_date').html('"From date" must be on the same day or after today');
      ...
      if(Date.parse(toDate) > Date.now()) {
        $('.error-to_date').html('"To date" must be on the same day or after today');
        $('.error-to_date').css('display','block');

        flag = true;
      } else if(Date.parse(fromDate) > Date.parse(toDate)) { 
        $('.error-to_date').html('"To date" must be on the same day or after "From date"');
        $('.error-to_date').css('display','block');
